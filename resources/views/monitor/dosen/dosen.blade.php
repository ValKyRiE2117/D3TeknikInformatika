<x-monitor-head  :tahunAjaranList="$tahunAjaranList">
    ...
    <div class="p-4 sm:ml-72 bg-gray-100 min-h-screen">
        ...
        <div class="p-4 bg-white rounded-lg shadow flex justify-between items-start lg:items-center flex-nowrap flex-col lg:flex-row"> 
            <h2 class="text-xl lg:text-2xl tracking-tight font-extrabold text-gray-700">List Dosen D3-Teknik Informatika</h2>
            @if(auth('dosen')->user()->role === 'kaprodi') <!-- Check if user is Kaprodi -->
            <div class="gap-2">
                <a href="/sistem-monitoring-mahasiswa/dosen/create" class="inline-flex items-center text-white bg-acorange-500 hover:bg-acorange-600 focus:ring-4 focus:ring-acorange-300 font-medium rounded-lg text-sm px-5 py-2 text-center transition">
                    Tambah Dosen
            ...
            <table id="search-table">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td class="font-medium text-gray-900 whitespace-nowrap">{{$dosen->nama_dosen}}</td>
                        <td>{{$dosen->npp}}</td>
                        <td class="whitespace-nowrap"><span class="
                            text-xs font-medium px-2.5 py-1 rounded-full 
                            {{ $dosen->role == 'kaprodi' ? 'bg-acgreen-300 text-acgreen-900' : '' }}
                            {{ $dosen->role == 'dosen' ? 'bg-acblue-200 text-acblue-900' : '' }}">
                            {{ ucfirst($dosen->role) }}
                        </span></td>
                        @if(auth('dosen')->user()->role === 'kaprodi') <!-- Check if user is Kaprodi -->
                        <td class="flex items-end px-6 py-4 gap-1">
                            ...
                            <form action="{{ route('dosen.destroy', $dosen->id) }}" method="POST" onsubmit="return confirm('Are you sure you want to delete this Dosen?')">
                                @csrf
                                @method('DELETE')
                ...
                <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                    ...
                    <div class="p-4 md:p-5 text-center">
                        ...
                        <h3 class="mb-5 text-md font-normal text-gray-500">
                            Apakah anda yakin ingin menghapus Matakuliah  <span id="delete-item-name" class="text-black">{{$dosen->nama_dosen}}</span>?
                        </h3>
                        <div class="flex justify-center">
                            <form id="delete-form" method="POST" action="{{ route('dosen.destroy', $dosen->id) }}">
                                @csrf
                                @method('DELETE')
