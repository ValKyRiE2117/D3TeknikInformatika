<x-monitor-head  :tahunAjaranList="$tahunAjaranList">
    ...
    <div class="p-4 sm:ml-72 bg-gray-100 min-h-screen">
        ...
        <div class="p-4 bg-white rounded-lg shadow flex justify-between items-start lg:items-center flex-nowrap flex-col lg:flex-row"> 
            <h2 class="text-xl lg:text-2xl tracking-tight font-extrabold text-gray-700">List Mahasiswa D3-Teknik Informatika</h2>
            
            @if(auth('dosen')->user()->role === 'kaprodi') <!-- Check if user is Kaprodi -->
                <div class="gap-2">
                    <!-- Tambah Data Mahasiswa Button -->
                    <a href="/sistem-monitoring-mahasiswa/mahasiswa/create" class="inline-flex items-center text-white bg-acorange-500 hover:bg-acorange-600 focus:ring-4 focus:ring-acorange-300 font-medium rounded-lg text-sm px-5 py-2 text-center transition">
                        Tambah Data Mahasiswa
                        <svg class="ml-1 w-5 h-5" fill="currentColor" viewBox="0 0 22 22" xmlns="http://www.w3.org/2000/svg"><g fill="currentColor" fill-rule="evenodd" clip-rule="evenodd"><path d="M2 12C2 6.477 6.477 2 12 2s10 4.477 10 10s-4.477 10-10 10S2 17.523 2 12m10-8a8 8 0 1 0 0 16a8 8 0 0 0 0-16"/><path d="M13 7a1 1 0 1 0-2 0v4H7a1 1 0 1 0 0 2h4v4a1 1 0 1 0 2 0v-4h4a1 1 0 1 0 0-2h-4z"/></g></svg>
                    </a>
        ...
        <div id="default-modal" tabindex="-1" aria-hidden="true" class="hidden overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 justify-center items-center w-full md:inset-0 h-[calc(100%-1rem)] max-h-full">
            <div class="relative p-4 w-full max-w-2xl max-h-full">
                ...
                <div class="relative bg-white rounded-lg shadow ">
                    ...
                    @csrf
                        <div class="p-4 md:p-5 space-y-4">
                            <div class="flex items-center justify-center w-full">
                                <label for="dropzone-file" class="flex flex-col items-center justify-center w-full h-38 border-2 border-gray-300 border-dashed rounded-lg cursor-pointer bg-gray-50  hover:bg-gray-100 ">
                            ...
                            <table class="w-full text-xs text-left rtl:text-right text-gray-500 ">
                                ...
                                <tbody>
                                    <tr class="">
                                        ...
                                        <td class="px-3 py-4 font-medium text-gray-900">
                                            *wajib sesuai nama lengkap dosen
                                        </td>
                                    </tr>
                                </tbody>
                            </table>
                    ...
                    <!-- Modal footer -->
                        <div class="flex items-end p-4 md:p-5 border-t border-gray-200 rounded-b ">
            ...
            <table id="search-table">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td class="font-medium text-gray-900 whitespace-nowrap">{{ $mhs->nama_mhs }}</td>
                        <td>{{ $mhs->nim }}</td>
                        <td class="whitespace-nowrap">{{ $mhs->dosenWali->nama_dosen ?? 'N/A' }}</td>
                        <td class="whitespace-nowrap">{{ $mhs->tahunAjaran->deskripsi ?? 'N/A' }}</td>
                        <td class="whitespace-nowrap"><span class="
                            text-xs font-medium px-2.5 py-1 rounded-full 
                            {{ $mhs->status == 'aktif' ? 'bg-acgreen-300 text-acgreen-900' : '' }}
                            {{ $mhs->status == 'tidak aktif' ? 'bg-acyellow-200 text-acyellow-900' : '' }}
                            {{ $mhs->status == 'lulus' ? 'bg-acblue-300 text-acblue-900' : '' }}
                            {{ $mhs->status == 'keluar' ? 'bg-acred-300 text-acred-900' : '' }}">
                            {{ ucfirst($mhs->status) }}
                        </span></td>
                        <td class="flex items-end px-6 py-4 gap-1">
                            ...
                            <a href="{{ route('mahasiswa.edit', $mhs->id) }}" class="text-white bg-acblue-500 hover:bg-acblue-600 focus:ring-4 focus:ring-acblue-200 font-medium rounded-lg text-sm px-4 py-2 focus:outline-none">Edit</a>
                            @if(auth('dosen')->user()->role === 'kaprodi') <!-- Check if user is Kaprodi -->
                            <form action="{{ route('mahasiswa.destroy', $mhs->id) }}" method="POST" onsubmit="return confirm('Are you sure you want to delete this Mahasiswa?')">
                                @csrf
                                @method('DELETE')
