<x-monitor-head :tahunAjaranList="$tahunAjaranList">
    ...
    <div class="p-4 sm:ml-72 bg-gray-100 min-h-screen">
        ...
        <div class="p-4 bg-white rounded-lg shadow flex justify-between items-start lg:items-center flex-nowrap flex-col lg:flex-row"> 
            <h2 class="text-xl lg:text-2xl tracking-tight font-extrabold text-gray-700">Tambah Mahasiswa</h2>
        </div>
        ...
        <div class="p-4 bg-white rounded-lg shadow flex justify-between items-start lg:items-center flex-nowrap flex-col lg:flex-row my-4">
            <form class="w-full" method="POST" action="{{ route('mahasiswa.store') }}">
                @csrf
                @method('POST')
                <div class="grid gap-2 md:gap-4 grid-cols-1 lg:grid-cols-12 mb-3">
                    <div class="col-span-12 lg:col-span-3">
                        ...
                        <input type="text" name="nim" id="nim" class="bg-gray-50 border border-gray-300 text-gray-900 text-md rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5" placeholder="A22.####.#####" required />
                    </div>
                    <div class="col-span-12 lg:col-span-6">
                        <label for="nama_lengkap" class="block mb-2 text-md font-medium text-gray-900">Nama Lengkap Mahasiswa</label>
                        <input type="text" name="nama_mhs" id="nama_lengkap" class="bg-gray-50 border border-gray-300 text-gray-900 text-md rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5" placeholder="" required />
                    </div>
                    <div class="col-span-12 lg:col-span-3">
                        <label for="semester" class="block mb-2 text-md font-medium text-gray-900">Semester</label>
                        <select id="semester" name="id_tahunajaran" class="bg-gray-50 border border-gray-300 text-gray-900 text-md rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5" placeholder="" required>
                            @foreach ($alltahunajaran as $tahunajar )
                            <option selected class="text-gray-500" value="{{ $tahunajar->id }}" >{{ $tahunajar->deskripsi }}</option>
                            @endforeach
                        </select>
                    </div>
                </div>
                <div class="grid gap-2 lg:gap-4 grid-cols-1 md:grid-cols-12 items-center">
                    ...
                    <div class="col-span-12">
                        ...
                        <select name="dosen_wali" id="doswal" class="bg-gray-50 border border-gray-300 text-gray-900 text-md rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5" placeholder="" required>
                            @foreach ($alldoswal as $doswal )
                            <option selected class="text-gray-500" value="{{ $doswal->id }}" >{{ $doswal->nama_dosen }}</option>
                            @endforeach
                        </select>
                    </div>
                </div>
