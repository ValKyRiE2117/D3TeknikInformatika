<x-monitor-head  :tahunAjaranList="$tahunAjaranList">
    ...
    <div class="p-4 sm:ml-72 bg-gray-100 min-h-screen">
        <nav class="flex mt-16 mb-4" aria-label="Breadcrumb">
            ...
            <li>
                <div class="flex items-center">
                  ...
                  <a href="#" class="ms-1 text-sm font-medium text-gray-700 hover:text-blue-600 md:ms-2">Edit Matakuliah</a>
                </div>
            </li>
            </ol>
        </nav>
        ...
        <div class="p-4 bg-white rounded-lg shadow flex justify-between items-start lg:items-center flex-nowrap flex-col lg:flex-row"> 
            <h2 class="text-xl lg:text-2xl tracking-tight font-extrabold text-gray-700">Edit Matakuliah: {{ old('nama_matkul', $matakuliah->nama_matkul) }}</h2>
        </div>
        <div class="p-4 bg-white rounded-lg shadow flex justify-between items-start lg:items-center flex-nowrap flex-col lg:flex-row my-4">
            <form class="w-full" method="POST" action="{{ route('matakuliah.update', $matakuliah->id) }}">
                @csrf
                @method('PUT')
                ...
                <div class="grid gap-2 lg:gap-4 grid-cols-1 md:grid-cols-12 items-center">
                    <div class="col-span-4">
                        ...
                        <input type="text" name="kelompok" id="kelompok" class="bg-gray-50 border border-gray-300 text-gray-900 text-md rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5" placeholder="A22.3101" required  value="{{ old('kelompok', $matakuliah->kelompok) }}"/>
                    </div>
                    <div class="col-span-4">
                    ...
                    <div class="col-span-4 flex pt-8">
                        <div class="flex items-center me-4">
                            <input 
                                ...
                                name="jenis" 
                                value="Teori" 
                                {{ old('jenis', $matakuliah->jenis) == 'Teori' ? 'checked' : '' }} 
                        ...
                        <div class="flex items-center me-4">
                            <input 
                                ...
                                name="jenis" 
                                value="Praktek" 
                                {{ old('jenis', $matakuliah->jenis) == 'Praktek' ? 'checked' : '' }} 
                                class="w-5 h-5 text-blue-600 bg-gray-100 border-gray-300 focus:ring-blue-500 focus:ring-2">
