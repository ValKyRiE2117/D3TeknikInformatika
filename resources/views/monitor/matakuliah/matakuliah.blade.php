<x-monitor-head  :tahunAjaranList="$tahunAjaranList">
    ...
    <div class="p-4 sm:ml-72 bg-gray-100 min-h-screen">
        ...
        <div class="p-4 bg-white rounded-lg shadow flex justify-between items-start lg:items-center flex-nowrap flex-col lg:flex-row"> 
            <h2 class="text-xl lg:text-2xl tracking-tight font-extrabold text-gray-700">List Matakuliah D3-Teknik Informatika</h2>
            <div class="gap-2">
                <a href="{{ route('matakuliah.create') }}" class="inline-flex items-center text-white bg-acorange-500 hover:bg-acorange-600 focus:ring-4 focus:ring-acorange-300 font-medium rounded-lg text-sm px-5 py-2 text-center transition">
        ...
        <div id="default-modal" tabindex="-1" aria-hidden="true" class="hidden overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 justify-center items-center w-full md:inset-0 h-[calc(100%-1rem)] max-h-full">
            <div class="relative p-4 w-full max-w-2xl max-h-full">
                ...
                <div class="relative bg-white rounded-lg shadow ">
                    <!-- Modal header -->
                    <div class="flex items-center justify-between p-3 md:p-4 border-b rounded-t ">
                    ...
                    @csrf
                        <div class="p-3 md:p-4 space-y-4">
                            <div class="flex items-center justify-center w-full">
                                <label for="dropzone-file" class="flex flex-col items-center justify-center w-full h-38 border-2 border-gray-300 border-dashed rounded-lg cursor-pointer bg-gray-50  hover:bg-gray-100 dark:border-gray-600">
                                    ...
                                    <input id="dropzone-file" name="file" accept=".xlsx,.xls" type="file" class="hidden" />
                                </label>
                            </div>
                        </div>
                        <div class="p-3 md:p-4 border-t border-gray-200">
                            <p class="mb-4 text-center text-md text-gray-900 font-medium">Silahkan Import file .xls berdasarkan ketentuan dibawah ini</p>
                            <table class="w-full text-xs text-left rtl:text-right text-gray-500 ">
                                <thead class="text-sm text-gray-600 bg-gray-200">
                                    <tr>
                        ...
                        <!-- Modal footer -->
                        <div class="flex items-end p-3 md:p-4 border-t border-gray-200 rounded-b ">
                            <button type="submit" class=" text-white bg-primary-600 hover:bg-primary-700 focus:ring-4 focus:ring-primary-200 font-medium rounded-lg text-sm px-4 py-2">
                                Import File
                            </button>
                        </div>
                    </form>
        ...
        <div class="p-4 bg-white mt-4 rounded-lg shadow"> 
          {{-- <h3 class="text-xl tracking-tight font-extrabold text-gray-700">Daftar Mahasiswa</h3> --}}
            <table id="search-table">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td class="font-medium text-gray-900 whitespace-nowrap">{{ $matakuliah->nama_matkul }}</td>
                        ...
                        <td class="flex items-center px-6 py-4 gap-1">
                            <!-- View Button -->
                            <a href="#" class="text-white bg-acgreen-500 hover:bg-acgreen-600 focus:ring-4 focus:ring-acgreen-200 font-medium rounded-lg text-sm px-4 py-2 focus:outline-none">Lihat</a>
                            
                            <!-- Edit Button -->
                            <a href="{{ route('matakuliah.edit', $matakuliah->id) }}" 
                                class="text-white bg-acblue-500 hover:bg-acblue-600 focus:ring-4 focus:ring-acblue-200 font-medium rounded-lg text-sm px-4 py-2 focus:outline-none">
                                 Edit
                             </a>
                            ...
                            {{-- <button 
                                ...
                                data-name="{{ $matakuliah->nama_matkul }}" 
                                class="delete-button text-white bg-acred-500 hover:bg-acred-600 focus:ring-4 focus:ring-acred-200 font-medium rounded-lg text-sm px-4 py-2 focus:outline-none">
                                Delete
                            </button> --}}
                            <form action="{{ route('matakuliah.destroy', $matakuliah->id) }}" method="POST" onsubmit="return confirm('Are you sure you want to delete this Matakuliah?')">
                                @csrf
                                @method('DELETE')
                ...
                <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                    ...
                    <div class="p-4 md:p-5 text-center">
                        ...
                        <h3 class="mb-5 text-md font-normal text-gray-500">
                            Apakah anda yakin ingin menghapus Matakuliah  <span id="delete-item-name" class="text-black">{{$matakuliah->nama_matkul}}</span>?
                        </h3>
                        <div class="flex justify-center">
                            <form id="delete-form" method="POST" action="{{ route('matakuliah.destroy', $matakuliah->id) }}">
                                @csrf
                                @method('DELETE')
                            ...
                            <button data-modal-hide="popup-modal-delete" type="button" class="py-2.5 px-5 ms-3 text-sm font-medium text-gray-900 focus:outline-none bg-white rounded-lg border border-gray-200 hover:bg-gray-100 focus:z-10 focus:ring-4 focus:ring-gray-100">Cancel</button>
                        </div>
